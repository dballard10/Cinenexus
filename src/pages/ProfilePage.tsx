...
import UserFavoriteItem from "@/components/profile/UserFavoriteItem";
...
const mockUser = {
  ...
  bio: "Movie and TV enthusiast. Always looking for the next great show to binge watch.",
};
...
const ProfilePage = () => {
  ...
  // Mock favorites - in a real app this would come from a user's personal favorites store
  const mockFavorites = trendingMovies?.slice(0, 4) || [];
              ...
              <Card className="p-6 bg-gray-800/50 backdrop-blur-md rounded-xl shadow-2xl border border-gray-700/50">
                ...
                {mockFavorites.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {mockFavorites.map((show) => (
                      <UserFavoriteItem key={show.id} show={show} />
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-8 text-gray-400">
                    ...
                    <p className="mt-2">
                      Explore shows and mark them as favorites!
                    </p>
                  </div>
                )}
              </Card>
